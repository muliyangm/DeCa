...
class CompositionDataset(Dataset):
    ...
    def __init__(
        ...
        # Clean only was here
        # attr2idx: {'ancient': 0, 'barren': 1, 'bent': 2, ...} [115]
        # obj2idx: {'aluminum': 0, 'animal': 1, 'apple': 2, ...} [245]
        self.obj2idx = {obj: idx for idx, obj in enumerate(self.objs)}
        self.attr2idx = {attr: idx for idx, attr in enumerate(self.attrs)}
        if self.open_world:
            self.pairs = self.full_pairs
        # all_pair2idx: {('ancient', 'building'): 0, ('ancient', 'castle'): 1, ('ancient', 'church'): 2, ..., } [1962]
        ...
        if subset:
            ...
            ind = ind[::len(ind) // 1000]  # start:end:step
